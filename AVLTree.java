public class AVLTree<T extends Comparable<T>> {
    ...
    private AVLNode insert(T x, AVLNode<T> t) {
        if( t == null )
            t = new AVLNode<>(x);
    ...
    */
    ...
    private void preOrder(AVLNode<T> node) {
        if (node != null) {
            System.out.print("  [" + node.dato + "] lvl: " + node.height);
    ...
    public T eliminar_min(AVLNode node) {
        if (node == null) {
            System.out.println("No se puede elimiar el valor");
        } else {
            if (node.izquierdo != null) {
                T x = eliminar_min(node.izquierdo);
            }
        }
    }*/
    ...
    public boolean remove(T value) {
        if (value == null)
            ...
            AVLNode<T> rem = search(root, value);
            if (rem == null)
                return false;
            else {
                AVLNode<T> dad = getFather(value);
                if (dad == null) { /*  Es el primer nodo  */
                    if (rem.izquierdo == null)
                        if (rem.derecho == null) { /*  No tiene hijos  */
                            /*  No hago nada  */
                        ...
                        if (rem.derecho == null) { /*  Tiene un hijo a la izquierda  */
                            root = rem.izquierdo;
                            rem.izquierdo = null;
                        } else {
    ...
    private AVLNode<T> getFather(T value) {
        if (value == null || root == null)
            return null;
        else if (value.equals(root.dato))
            return null;
        else
            return getFatherPart2(root, value);
    }
    
    private AVLNode<T> getFatherPart2(AVLNode<T> node, T value) {
        if (value.compareTo(node.dato) == 1) {
            if (node.derecho == null)
                return null;
            else if (value.equals(node.derecho.dato))
                return node;
            else
                return getFatherPart2(node.derecho, value);
        } else {
            if (node.izquierdo == null)
                return null;
            else if (value.equals(node.izquierdo.dato))
                return node;
            else
                return getFatherPart2(node.izquierdo, value);
        }
    }
    ...
    private AVLNode<T> search(AVLNode<T> node, T value) {
        if (value.equals(node.dato))
            return node;
        else if (value.compareTo(node.dato) == 1)
            if (node.derecho == null)
                return null;
            else
                return search(node.derecho, value);
        else
            if (node.izquierdo == null)
                return null;
            else
                return search(node.izquierdo, value);
    }
}